...
def index_corpus(
    search_engine: SearchEngine, corpus: List[str], cache_path: str = None
):
...
def main():
    ...
    base_results = run_queries(base_search_engine, queries, top_k=10)
    ...
    finetuned_results = run_queries(finetuned_search_engine, queries, top_k=10)
    ...
    for metric_name in base_metrics.keys():
        ...
        improvement = (
            ((finetuned_val - base_val) / base_val * 100) if base_val > 0 else 0
        )
